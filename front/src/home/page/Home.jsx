...
export const Home = () => {
  ...
  const getParams = async () => {
    if (token != "null" || user != "null") {
      ...
      const url = `http://localhost:${PORT}/api/LinkedIn`
      const urlImg = `http://localhost:${PORT}/api/LinkedIn/img`
      const urlEmail = `http://localhost:${PORT}/api/LinkedIn/email`
      ...
      const photoURL = image.displayImage.elements[0].identifiers[0].identifier
  ...
  const postLinkedIn = async (e) => {
    ...
    if (postValueInput != "") {
      const dataText = {
        message: postValueInput,
        token: token,
        userid: user.userid,
      }
      const url = `http://localhost:${PORT}/api/LinkedIn/post`
      try {
